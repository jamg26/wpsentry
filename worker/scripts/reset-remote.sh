#!/bin/bash
# Full factory reset of all remote Cloudflare resources (interactive).
# Usage: npm run reset:remote  (from worker/)
set -e

ACCOUNT_ID="${CLOUDFLARE_ACCOUNT_ID:?Error: CLOUDFLARE_ACCOUNT_ID env var is not set. Run: export CLOUDFLARE_ACCOUNT_ID=<your-id>}"

echo "⚠️  WARNING: This will wipe ALL remote resources:"
echo "   • D1 database rows (usage, api_keys, scans, users)"
echo "   • KV: SESSIONS_KV, RATELIMIT_KV"
echo "   • R2: all objects in jwp-scan-reports"
echo "   • Queues: jwp-scan-jobs / jwp-scan-jobs-dlq (see step 4)"
echo ""
